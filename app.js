...
const number=n=>Number(n.toFixed(2)).toLocaleString();
...
const readable=(d,options={month:'short',day:'numeric'})=>date(d).toLocaleDateString('en-US',options);
...
function render(){
  ...
  $('save-status').textContent=storageOK?'Saved on this device':'Storage unavailable — keep this tab open';
  ...
  $('hours-label').firstChild.textContent=s.mode==='remaining'?'Hours left to work':'Total hours required';$('completed-label').hidden=s.mode!=='total';
  $('daily').textContent=c.days.length?number(c.daily):'—';
  const minutes=Math.ceil(c.daily*60);
  $('daily-detail').textContent=!c.days.length?(c.remaining?'No available workdays. Adjust your dates.':'Target already met.'):`${Math.floor(minutes/60)}h ${minutes%60}m each workday`;
  ...
  $('holiday-status').textContent=holidayMessage;
  const codes=[...new Set(holidayRows.flatMap(h=>h.subdivisionCodes||[]))].sort();
  $('subdivision-label').hidden=!codes.length;
  $('subdivision').innerHTML='<option value="">National holidays only</option>'+codes.map(code=>`<option value="${escape(code)}">${escape(code)}</option>`).join('');
  $('subdivision').value=s.subdivision;
  $('weekdays').innerHTML=['S','M','T','W','T','F','S'].map((name,i)=>`<button class="${s.weekdays.includes(i)?'active':''}" aria-label="${['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][i]}" aria-pressed="${s.weekdays.includes(i)}" data-weekday="${i}">${name}</button>`).join('');
  ...
  let html=Array.from({length:7},(_,i)=>['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][(s.weekStartsOn+i)%7]).map(n=>`<div class="day-name">${n}</div>`).join('');
  ...
  html+=days.map(d=>{
    ...
    const label=h[d] || (vacation?'Paid vacation':working?number(c.daily)+'h target':past?'':'Day off');
    return `<button class="day ${!working?'off':''} ${past?'past':''} ${h[d]?'holiday':vacation?'vacation':''} ${d===today?'today':''}" data-date="${d}" title="${escape(readable(d)+' — '+label)}" aria-label="${escape(readable(d)+' — '+label+'; edit day')}"><span class="date-number">${date(d).getDate()}</span><small>${escape(label)}</small></button>`;
  }).join('');
  ...
  $('planned-summary').textContent=`${number(c.planned)} / ${number(c.remaining)}h planned`;
  $('progress-bar').style.width=`${c.remaining?Math.min(100,c.planned/c.remaining*100):100}%`;
  const notices=[plannedMessage(c.planned,c.remaining)];
  const excluded=s.blocks.filter(b=>b.type==='work' && b.date.startsWith(s.month) && !c.days.includes(b.date));
  if(excluded.length)notices.push(`${excluded.length} work block(s) outside available days are excluded from totals.`);
  const overLimit=c.days.filter(d=>s.blocks.filter(b=>b.type==='work'&&b.date===d).reduce((n,b)=>n+b.end-b.start,0)>s.limit+.001);
  if(overLimit.length)notices.push(`${overLimit.length} planned day(s) exceed your daily limit.`);
  $('plan-notice').textContent=notices.join(' ')||'All your hours have a home. You’re set.';
  $('clear-blocks').disabled=!s.blocks.length;
  $('undo-clear').hidden=!clearedBlocks;
  $('auto').disabled=!c.days.length||!c.unplanned;
  let grid='<div class="schedule-head"></div>'+wd.map(d=>`<div class="schedule-head">${readable(d,{weekday:'short'})}<strong>${date(d).getDate()}</strong><small>${escape(h[d]|| (s.vacation.includes(d)?'Paid vacation':number(s.blocks.filter(b=>b.date===d&&b.type==='work').reduce((n,b)=>n+b.end-b.start,0))+'h planned'))}</small></div>`).join('');
...
function newPreset(){return {...presets[preset],label:$('quick-label').value.trim()||presets[preset].label,duration:Number($('quick-duration').value)};}
...
function placeBlock(block,d,start){
  ...
  let error='';
  if(!d.startsWith(s.month))error='Choose a date in the planning month.';
  else if(start<0||candidate.end>24)error='The whole block must fit within this day.';
  else if(candidate.type==='work'&&!calculate(s,holidays()).days.includes(d))error='Work can only be placed on an available workday.';
  else if(overlaps(candidate,s.blocks))error='That time is occupied. Drop into a free slot.';
  if(error){$('placement-status').textContent=error;return;}
  s.blocks=s.blocks.filter(b=>b.id!==candidate.id).concat(candidate);save();
  $('placement-status').textContent=`${candidate.label} ${block.id?'moved':'placed'}: ${readable(d)}, ${time(start)}–${time(candidate.end)}.`;
}
...
document.querySelectorAll('[data-preset]').forEach(button=>{
  button.onclick=()=>{
    preset=button.dataset.preset;$('quick-label').value=presets[preset].label;$('quick-duration').value=presets[preset].duration;
    document.querySelectorAll('[data-preset]').forEach(b=>b.setAttribute('aria-pressed',b===button));
  };
  button.onpointerdown=e=>beginDrag(e,button.dataset.preset===preset?newPreset():presets[button.dataset.preset]);
});
function openDay(d){
  selectedDay=d;$('day-title').textContent=readable(d,{weekday:'long',month:'long',day:'numeric'});
  $('day-detail').textContent=holidays()[d]||'Choose whether this day is available for work.';
...
$('save-day').onclick=()=>{
  ...
  if($('day-type').value==='holiday')s.custom[selectedDay]=$('holiday-name').value.trim()||'Custom holiday';
  save();
};
function openBlock(b=null,d=null,start=9){
  editId=b?.id||null;$('block-title').textContent=b?'Edit time block':'Add time block';
  $('block-label').value=b?.label||'Work';$('block-type').value=b?.type||'work';
...
$('block-type').onchange=()=>{if(['Work','Break'].includes($('block-label').value))$('block-label').value=$('block-type').value==='work'?'Work':'Break';};
$('block-form').onsubmit=e=>{
  ...
  if(!block.label)return fail('Enter a title for this block.');
  if(block.end<=block.start)return fail('End time must be after start time. Split overnight blocks across two dates.');
  if(!block.date.startsWith(s.month))return fail('Choose a date in the planning month.');
  ...
  if(!dates.length)return fail('There are no available workdays in that week.');
  const candidates=dates.map((d,i)=>({...block,id:i?crypto.randomUUID():block.id,date:d}));
  if(block.type==='work'&&candidates.some(b=>!available.includes(b.date)))return fail('This is not an available workday. Change your start date, working week, or time off first.');
  if(candidates.some(b=>overlaps(b,s.blocks)))return fail('This time overlaps another block. Choose a free time or edit the existing block.');
...
$('clear-blocks').onclick=()=>{
  ...
  $('placement-status').textContent=`Cleared all ${clearedBlocks.length} blocks across all dates. Undo is available until you reload.`;
};
$('undo-clear').onclick=()=>{
  if(clearedBlocks.some(b=>overlaps(b,s.blocks))){$('placement-status').textContent='Some new blocks overlap the cleared plan. Move or delete those new blocks before restoring.';return;}
  s.blocks.push(...clearedBlocks.filter(b=>!s.blocks.some(existing=>existing.id===b.id)));
  clearedBlocks=null;save();$('placement-status').textContent='Cleared blocks restored.';
};
...
$('auto').onclick=()=>{
  ...
  if(s.workEnd<=s.workStart){$('placement-status').textContent='Finish time must be after the start time (00:00 means midnight).';return;}
  const lunch={enabled:$('auto-lunch').checked,start:fromTime($('lunch-start').value),duration:Number($('lunch-duration').value)};
  if(!Number.isFinite(lunch.start)||lunch.start+lunch.duration>24){$('placement-status').textContent='Choose a lunch time that fits within the day.';return;}
  const result=planWithLunch(s,holidays(),lunch);s.blocks=result.blocks;
  week=weekStart(c.days[0]);save();
  $('placement-status').textContent=result.skipped.length?`Lunch could not be added on ${result.skipped.map(d=>readable(d)).join(', ')} because existing blocks occupy that time. Move those blocks first.`:lunch.enabled?'Hours distributed with room for lunch.':'Hours distributed.';
};
$('example').onclick=()=>{
  if(s.blocks.length && !confirm('Replace this plan with the September example?'))return;
  clearedBlocks=null;
...
async function loadHolidays(){
  ...
  if(seaCountries.includes(region)){
    try{
      const response=await fetch('./sea-holidays.json');
